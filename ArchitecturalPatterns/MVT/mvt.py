...
class TaskView:
    ...
    def show_all_tasks(self):
        ...
        self.template.show_tasks(tasks)

    def add_task(self):
        title = self.template.get_task_title()
        if title.strip():
            self.model.add_task(title)
            self.template.show_message("Завдання додано!")
        else:
            self.template.show_error("Назва завдання не може бути порожньою")

    def complete_task(self):
        task_id = self.template.get_task_id()
        if task_id:
            if self.model.complete_task(task_id):
                self.template.show_message("Завдання позначено як виконане!")
            else:
                self.template.show_error("Завдання з таким ID не знайдено")
        else:
            self.template.show_error("Невірний ID завдання")

    def delete_task(self):
        task_id = self.template.get_task_id()
        if task_id:
            if self.model.delete_task(task_id):
                self.template.show_message("Завдання видалено!")
            else:
                self.template.show_error("Завдання з таким ID не знайдено")
        else:
            self.template.show_error("Невірний ID завдання")
...
class TodoApp:
    ...
    def __init__(self):
        self.controller = TaskView()

    def run(self):
        while True:
            self.controller.template.show_menu()
